    ...
    <v-container>
      <div style="margin-top: 90px; display: flex; width: 100% !important;">
        ...
        <div style="width: -webkit-fill-available">
          <div class="mt-5">
            <div :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'" class="button-container"
              style="display: flex; border-radius: 10px; width: fit-content; height: 65px; align-items: center; padding: 10px;">
              <v-btn class="me-3" :class="[selectedScreen ? 'wallet-btn' : isDark ? 'fiat-btn' : 'fiat-btn-light']"
                @click.prevent="selectedScreen = true">Crypto Wallet </v-btn>
              <v-btn :class="[!selectedScreen ? 'wallet-btn' : isDark ? 'fiat-btn' : 'fiat-btn-light']"
                @click.prevent="selectedScreen = false">Fiat Wallet</v-btn>
            </div>
            ...
            <div v-if="selectedScreen" class="wallet-box" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
              ...
              <v-table style="display: grid! important; background: inherit; width: 100%;">
                <thead>
                  <tr style="display: flex; margin-bottom: 8px; justify-content: space-between;">
                    ...
                    <th class="me-7 coin-th"
                      style="display: flex; align-items: center; align-self: center; width: 20%; justify-content: center;">
                      <div class="d-flex">
                        <span class="table-header-text" :class="isDark ? 'text-dark' : 'text-light'">Coin</span>
                      </div>
                    </th>

                    <th class="me-7 price-th"
                      style="display: flex; align-items: center; align-self: center; position: relative;  width: 12%;">
                    ...
                    <th class="flex-lg-and-up hidden-sm-and-down"
                      style="display: flex; align-items: center; align-self: center; position: relative; margin-right: 21px">
                      <span class="table-header-text" :class="isDark ? 'text-dark' : 'text-light'"
                        style="margin-left: ">Quantity</span>
                    </th>

                    <th class="me-5"
                      style="display: flex; align-items: center; align-self: center; position: relative; width: 27%;">
                      <span class="table-header-text me-1" :class="isDark ? 'text-dark' : 'text-light'"
                ...
                <tbody>
                  ...
                  <tr @click.stop="pinia.state.getNewCoinInfo = token.symbol; navigateTo('/account/trade/coinId')"
                    class="token-price py-2" :class="isDark ? 'wallet-border' : 'wallet-border-light'"
                    v-for="token in pinia.state.tokenLists.slice(0, sliceEndValue)" :key="token.id"
                    ...
                    <td style="display: contents; cursor: pointer;">
                      ...
                      <div class="d-flex me-7 coin-wrap" style="align-items: center; width: 25%;">
                        ...
                        <img :src="chainIcon?.icon" width="15" class="chainIcon"
                          style="position: relative; right: 6%; margin-top: 16px;" />
                        <div class="coin-div"
                          style="flex-direction: column; display: flex; overflow: hidden; text-overflow: ellipsis;">
                    ...
                    <td class="mt-2 coin-price-td"
                      style="display: flex; align-items: center; width: 21%; cursor: pointer;"><span
                        class="browser-txt coin-price" style="display: flex; align-self: start; margin-top: 3px;"
                        :class="isDark ? 'coin-name' : 'coin-name-light'">{{ formatBalance(token?.conversionRate)
                        }}</span></td>

                    <td class="quantity-div" style="display: flex; align-items: end; width: 15%; cursor: pointer;">
                      <span class="browser-txt mb-2 flex-md-and-up hidden-sm-and-down"
                        :class="isDark ? 'coin-name' : 'coin-name-light'"
                        style="font-weight: 700; display: flex; align-self: center;">
                    ...
                    <div class="d-flex" style="align-items: center;">
                        ...
                        <v-dialog max-width="500">
                          <template v-slot:activator="{ props: activatorProps }">
                            <v-btn v-bind="activatorProps" style="background: inherit; box-shadow: none;">
                              <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25"
                                fill="none">
                                <path
                                  d="M14.5 5.5C14.5 4.4 13.6 3.5 12.5 3.5C11.4 3.5 10.5 4.4 10.5 5.5C10.5 6.6 11.4 7.5 12.5 7.5C13.6 7.5 14.5 6.6 14.5 5.5Z"
                                  stroke="#D8D8D8" stroke-width="1.5" />
                                <path
                                  d="M14.5 19.5C14.5 18.4 13.6 17.5 12.5 17.5C11.4 17.5 10.5 18.4 10.5 19.5C10.5 20.6 11.4 21.5 12.5 21.5C13.6 21.5 14.5 20.6 14.5 19.5Z"
                                  stroke="#D8D8D8" stroke-width="1.5" />
                                <path
                                  d="M14.5 12.5C14.5 11.4 13.6 10.5 12.5 10.5C11.4 10.5 10.5 11.4 10.5 12.5C10.5 13.6 11.4 14.5 12.5 14.5C13.6 14.5 14.5 13.6 14.5 12.5Z"
                                  stroke="#D8D8D8" stroke-width="1.5" />
                              </svg>
                            </v-btn>
                          </template>
                          ...
                          <template v-slot:default="{ isActive }">
                            <v-card max-width="400" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                              ...
                              <h4 style="font-family: Manrope; font-size: 400 !important;">Select the action you want to
                                perform</h4>
                              <div class="d-flex me-7 mt-4" style="align-items: center;">
                                ...
                                <div style="display: flex; justify-content: space-between;">
                                  <div class="coin-div" style="flex-direction: column; display: flex !important;">
                                    ...
                                    <span class="sml-text" :class="isDark ? 'text-dark' : 'text-light'"
                                  ...
                                  <h3
                                    style="position: absolute; right: 7%; font-family: Manrope; font-weight: 700; font-size: 16px;">
                                    {{ formatBalance(token.balance) }}</h3>
                                </div>
                              </div>
                              ...
                              <div class="d-flex mt-6" style="justify-content: space-between;">
                                <td class="me-2" style="display: flex; align-items: center; width: 100%">
                                  ...
                                  <v-btn @click.once.stop="navigateTo('/account/trade/sendCoin')" class="swap"
                                ...
                                <td class="me-2" style="display: flex; align-items: center; width: 100%">
                                  ...
                                  <v-btn @click.prevent="navigateTo('/account/trade/getCoin')" class="swap"
                                    :class="isDark ? 'btn-segment' : 'btn-segment-light'">
                                ...
                                <td style="display: flex; align-items: center; color: white; width: 100%">
                                  ...
                                  <v-btn @click.prevent="navigateTo('/account/trade/swap')"
                                    :class="isDark ? 'btn-segment' : 'btn-segment-light'" class="swap">
                      ...
                      <td class="flex-md-and-up hidden-sm-and-down me-3"
                        style="display: flex; align-items: center; color: white;">
                        <div>
                          <v-btn @click.prevent="navigateTo('/account/trade/sendCoin')" class="swap"
                            :class="isDark ? 'btn-segment' : 'btn-segment-light'">
                            <img src="/svg/send-arrow.svg" class="me-1" />
                      ...
                      <td class="flex-md-and-up hidden-sm-and-down"
                        style="display: flex; align-items: center; color: white;">
                        <div>
                          <v-btn class="swap" @click.prevent="navigateTo('/account/trade/swap')"
                            :class="isDark ? 'btn-segment' : 'btn-segment-light'">
              ...
              <div v-show="pinia.state.tokenLists.length > 4"
                style="width: 100%; display: flex; justify-content: center; margin-top: 5px; padding: 0px">
                <v-btn variant="text" @click="toggleTokens" style="text-transform: capitalize; padding: 0px 5px"> {{
                  showAll ? 'See Less' : 'See More' }}</v-btn>
              </div>
            </div>
            ...
            <div style="margin-top: 47px; margin-bottom: 34px;">
              <span :class="isDark ? 'card-text-dark' : 'card-text-light'"
                style="font-family: Manrope; font-size: 24px; font-style: normal; font-weight: 400; line-height: normal;">Transaction
                History</span>
            </div>
...
.notivue-notification {
  ...
  /* Set this value higher than Vuetify modal */
}
