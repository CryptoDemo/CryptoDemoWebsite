          ...
          <div style="margin-bottom: 113px; margin-top: 110px;display: flex;">
            ...
            <div>
              <div class="settings-body">
               ...
               <v-row no-gutters style="margin-top: 32px; width: 101%; margin-bottom:16px">
                    <v-col md="" sm="5" class="username-col" style="margin-inline-end: 18px;">
                      <div class="d-flex img-col" style="margin-top: 0px!important; margin-left: 16px;">
                        ...
                        <div class="email-div">
                         <v-text-field placeholder="Username" color="success"  :loading="loading" class="input-styling1 pl-4" :class="isDark ? 'profile-cards-dark':'profile-cards-light'" variant="plain" style="text-align: justify; font-family: Poppins; font-size: 14px; font-style: normal; font-weight: 400;">
                  ...
                  <v-row class="user-inputs d-sm-flex" style="margin-left: 16px">
                  <v-col md="6" cols="12">
                    <input type="tel" id="phone" placeholder="333-4444-4444" minlength="9" maxlength="12" v-model="phoneNumber" style="font-size: 16px; font-weight: 400; height: 64px; outline: none; width: 100%" class="input-styling1 pl-4 pr-4" :class="isDark ? 'profile-cards-dark':'profile-cards-light'"/>
                  </v-col>
                  <v-col :class="isDark ? 'date-input-profile-cards-dark':'date-input-profile-cards-light'">
                   <v-date-input prepend-icon="" clearable class="dateInput" v-model="DateOfBirth" variant="plain"></v-date-input>
                  </v-col>
                  </v-row>
...
const ChangeUsername = async () => {
  ...
  if (data.success) {
    // pinia.setUser(user)
    push.success('username changed succesfully')
    loading.value = false 
  } else{
    loading.value = false 
...
const UpdateUserInfo = async () => {
  ...
  if (data.success) {
    const user = { ...info, token: userToken };
    pinia.setUser(user);
    push.success('user details updated succesfully')
    loading.value = false 
  } else{
    push.error(data.message, { duration: 9000 })
  }
