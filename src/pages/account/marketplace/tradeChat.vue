...
const create_Dispute = async () => {
  ...
  const payload = {
    seller_id: selectedCoin.value.buyer_id,
    offer_id: selectedCoin.value.offer_id,
    order_id: selectedCoinId,
    message: disputeMsg.value,
    file_urls: [file_url.value],
    title: disputeType.value
  };

  console.log(payload)

  try {
    const data = await createDispute(payload);
    if (data.success) {
      loading.value = false;
      push.success('Dispute has been Successfully created')

    } else {
      push.error(`${data.message}`);
      loading.value = false;
      push.error("failed to create dispute")
    }
  } catch (e) {
    console.log(e);
    loading.value = false;
...
const disputeType = ref('Payment Not Received'); // Selected dispute type
const disputeTitles = [
  { dispute_name: 'Payment Not Received' },
  { dispute_name: 'Received Wrong Amount' },
  { dispute_name: 'Buyer Sent Funds But Not Verified' },
  { dispute_name: 'Seller Didn’t Release Crypto' },
  { dispute_name: 'Payment Method Issue' },
  { dispute_name: 'Account Freezed After Payment' },
  { dispute_name: 'Fake Payment Confirmation' },
  { dispute_name: 'Fraudulent P2P Transaction' },
  { dispute_name: 'Partial Payment Sent' },
  { dispute_name: 'Dispute Over Exchange Rate' },
  { dispute_name: 'Seller is Unresponsive' },
  { dispute_name: 'Buyer Reversed Payment' },
  { dispute_name: 'Payment Held by Third Party' },
  { dispute_name: 'Unauthorized Account Use' },
  { dispute_name: 'Other P2P Transaction Issue' },
];
