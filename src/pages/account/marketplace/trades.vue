            ...
            <div class="ml-4" style="width: 100%;">
                ...
                <div class="mt-5" style="height: 500px; overflow-y: scroll; margin-bottom: 150px;">
                    ...
                    <div v-if="!filteredOrders.length" style="display: flex; flex-direction: column; justify-content: center; margin: auto; margin-top: 70px;">
                        ...
                        <path d="M18.04 13.55C17.62 13.96 17.38 14.55 17.44 15.18C17.53 16.26 18.52 17.05 19.6 17.05H21.5V18.24C21.5 20.31 19.81 22 17.74 22H6.26C4.19 22 2.5 20.31 2.5 18.24V11.51C2.5 9.44001 4.19 7.75 6.26 7.75H17.74C19.81 7.75 21.5 9.44001 21.5 11.51V12.95H19.48C18.92 12.95 18.41 13.17 18.04 13.55Z" fill="0B6B96" stroke="#2873FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M2.5 12.4113V7.84132C2.5 6.65132 3.23 5.59128 4.34 5.17128L12.28 2.17128C13.52 1.70128 14.85 2.62131 14.85 3.95131V7.7513" stroke="#2873FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M22.5588 13.9692V16.0292C22.5588 16.5792 22.1188 17.0292 21.5588 17.0492H19.5988C18.5188 17.0492 17.5288 16.2592 17.4388 15.1792C17.3788 14.5492 17.6188 13.9592 18.0388 13.5492C18.4088 13.1692 18.9188 12.9492 19.4788 12.9492H21.5588C22.1188 12.9692 22.5588 13.4192 22.5588 13.9692Z" stroke="#0B6B96" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M7 12H14" stroke="#2873FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    ...
                    <v-card v-for="(order, index) in filteredOrders" :key="index" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'" style="padding: 20px; border-radius: 15px; margin-bottom: 10px;">
                        <span style="font-size: 14px">Trade time span <span :class="{'expired-text': order.status === 'expired','active-text': order.status !== 'expired'}" style="font-size: 14px;">{{ formatTime(order.created_at) }} - {{ formatTime(order.expires_in) }}</span></span>
                        ...
                        <div style="display: flex; flex-direction: column; align-items: end;" :class="isDark ? 'text-dark':'text-light'">
                            ...
                            <span :class="{'expired-text': order.status === 'expired','active-text': order.status !== 'expired'}">{{ order.status }}</span>   
                            <span>Buyer</span>
                            <div>
