    ...
    <v-container class="mkt-ctxn" style="display: flex; margin-top: 110px;">
      ...
      <div class="offer-body" style="width: 100%; margin-left: 28px;">
        <div class="acct-settings" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
          style="display: flex; justify-content: space-between; margin-bottom: 20px; border: none">
          <span class="marketPlace"
            style="font-size: 24px; font-style: 28px; font-weight: 600; color: #5892FF;">Marketplace</span>
        ...
        <div class="d-flex">
          ...
          <div class="offer-container" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
            style="margin-bottom: 300px;  padding-top: 2px; width: 70%; overflow: scroll; padding-left: 16px; padding-right: 16px">

            <div class="button-container py-3" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
              style="display: flex; border-radius: 10px; align-items: center;">
              <v-btn class="me-3" :class="[selectedScreen ? 'personalBtn' : isDark ? 'marketBtn' : 'marketBtn-light']"
                @click.prevent="selectedScreen = true">Market Offers</v-btn>
              <v-btn :class="[!selectedScreen ? 'personalBtn' : isDark ? 'marketBtn' : 'marketBtn-light']"
                @click.prevent="selectedScreen = false">
                Personal Offers
                <!-- Render the span only if offersCount is not empty -->
                <span v-if="pinia.state.offersCount"
                  style="background: orangered; color: white; padding: 8px; border-radius: 15px; margin: 10px; height: 23px; display: flex; align-items: center; font-size: 12px">
              ...
              <v-menu>
                <template v-slot:activator="{ props }">
                  <button v-bind="props">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                      class="bi bi-filter-circle-fill hidden-lg-and-up flex-sm-and-down ml-6" viewBox="0 0 16 16">
                      <defs>
                ...
                <v-list @click.stop="" style="border-radius: 10px; width: 250px; padding: 10px"
                  ...
                  <v-menu>
                    <template v-slot:activator="{ props }">
                      <v-btn @click.prevent="toggleChevron()" v-bind="props"
                        :class="isDark ? 'offers-cards-dark' : 'offers-cards-light'"
                        style="width: fit-content; height: 50px; margin-top: 8px; border-radius: 10px;  box-shadow: none; letter-spacing: 0px;width: 100%; display: flex; justify-content: space-between; text-transform: capitalize;">
                  ...
                  <v-menu>
                    ...
                    <v-list :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                      style="border-radius: 15px; margin-top: 10px; margin-top: 10px; max-height: 300px; overflow-y: auto; width: 225px;">
                        ...
                        <v-row dense style="max-width: 240px; height: 250px; overflow: scroll;">
                          <div v-for="(range, index) in priceLimits" :key="index" style="width: 100%;">
                            ...
                            <v-list-item @click="setSelectedLimit(range)" style="display: flex;">
                              ...
                              <span class="currency-list">{{ range.min }} - {{ range.max === Infinity ? 'Infinity' :
                                range.max }}</span>
                  ...
                  <v-btn @click="resetFilters()" class="primary-btn1"
                    style="width: 100%; color: white; border-radius: 8px !important; font-weight: 600;">Clear
                    Filters</v-btn>
            ...
            <div v-if="selectedScreen" class="offers-div d-flex"
              style="overflow: scroll; flex-direction: column; justify-content: space-between;">



              <div class="mb-3 px-4 pt-3" v-for="offer in filteredOffers" :key="offer.id"
                :class="isDark ? 'offers-cards-dark' : 'offers-cards-light'" style="padding-bottom: 2px">

                <div
                  style="align-items: center; border: none; align-items: baseline; display: flex; justify-content: space-between;">
                  <div>
                    <div style="display: flex; align-items: center; line-height: 30px;">
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="#8e9bae"
                        class="bi bi-person-bounding-box" viewBox="0 0 16 16" v-else>
                        <path
                          d="M1.5 1a.5.5 0 0 0-.5.5v3a.5.5 0 0 1-1 0v-3A1.5 1.5 0 0 1 1.5 0h3a.5.5 0 0 1 0 1zM11 .5a.5.5 0 0 1 .5-.5h3A1.5 1.5 0 0 1 16 1.5v3a.5.5 0 0 1-1 0v-3a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 1-.5-.5M.5 11a.5.5 0 0 1 .5.5v3a.5.5 0 0 0 .5.5h3a.5.5 0 0 1 0 1h-3A1.5 1.5 0 0 1 0 14.5v-3a.5.5 0 0 1 .5-.5m15 0a.5.5 0 0 1 .5.5v3a1.5 1.5 0 0 1-1.5 1.5h-3a.5.5 0 0 1 0-1h3a.5.5 0 0 0 .5-.5v-3a.5.5 0 0 1 .5-.5" />
                        <path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm8-9a3 3 0 1 1-6 0 3 3 0 0 1 6 0" />
                      </svg>
                      <span class="me-3 ml-2" style="font-size: 14px; font-weight: 600; text-transform: capitalize">{{
                    ...
                    <div class="d-flex" style="line-height: 30px;">
                      ...
                      <span class="me-1"
                        style="color: #8e9bae; font-family: Manrope; font-size: 14px; font-style: normal; font-weight: 600;">{{
                          offer.trading_pair?.crypto?.token?.name
                        }}</span>
                    </div>

                    <div style="display: flex; flex-direction: column; line-height: 30px;">
                      <span class="flex-lg-and-up hidden-sm-and-down mkt-values"
                        :class="isDark ? 'text-dark' : 'text-light'">Minimum-Maximum buy limit</span>
                      <span class="hidden-lg-and-up flex-sm-and-down mkt-values"
                        :class="isDark ? 'text-dark' : 'text-light'">buy
                        limit</span>
                      ...
                      <div class="d-flex">
                        ...
                        <span v-if="!pinia.state.user.kyc_verified"
                          style="font-family: Manrope; font-size: 14px; font-style: normal; margin-left: 4px; font-weight: 500;  color: orangered ">Get
                          Verified to purchase coin</span>
                      </div>
                    </div>
                  ...
                  <div style="display: flex; flex-direction: column; justify-content: flex-end; line-height: 30px;">
                    <span v-if="offer.user?.is_verified" class="resend-code"
                      style="font-weight: 600;  text-align-last: right; font-size: 14px;">Verified</span>
                    ...
                    <span :class="isDark ? 'text-dark' : 'text-light'"
                      style=" font-family: Manrope; font-size: 14px; font-style: normal;  font-weight: 600; text-align-last: right;">{{
                ...
                <div class="mb-3">
                  <v-dialog max-width="500">
                    <template v-slot:activator="{ props: activatorProps }">
                      <v-btn
                        @click.prevent="pinia.state.selected_coin_to_buy_from_marketplace = offer?.id; pinia.state.selected_coin_to_buy_from_marketplace_userID = offer?.user?.id"
                        :disabled="!pinia.state.user.kyc_verified" v-bind="activatorProps"
                        class="smaller-btn mt-2 mb-4">Buy Offer</v-btn>
                    </template>
                    ...
                    <template v-slot:default="{}">
                      <v-card :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                        ...
                        <v-card-text>
                          ...
                          <span style="font-size: 14px;">By continuing, you will be buying <span
                              style="font-weight: 600;">{{ offer.trading_pair?.crypto?.token?.name }}</span> from {{
                                offer?.user?.username }}</span>
                          <input type="number" placeholder="Enter Amount to pay" v-model="amount_to_pay"
                            :class="isDark ? 'txn-cards-dark' : 'txn-cards-light'"
                            style="outline: none; height: 60px; padding-right: 25px!important; position: relative; border-radius: 15px; width: 100%; margin-top: 10px; padding-left: 15px;" />

                          <v-btn :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                            style="min-width: 70px; height: 56px; position: absolute; margin-top: 12px; border-radius: 15px; box-shadow: none; right: 26px; letter-spacing: 0px;  text-transform: capitalize;">
                          ...
                          <span style="font-size: 14px;" :class="isDark ? 'text-dark' : 'text-light'">Range Unit Value
                            <span style="font-weight: 600; font-size: 14px;"
                              :class="isDark ? 'card-title' : 'card-title--light'">{{
                                offer?.trading_pair?.fiat?.minimum_buy_limit
                              }} - {{ offer?.trading_pair?.fiat?.maximum_buy_limit }} {{ offer?.countryCurrencyName }}
                            </span>
                          </span>

                          <div class="mt-5">
                            <span style="font-weight: 600; font-size: 14px;">Expected amount you will receive</span>
                            <input type="number" disabled placeholder="Amount to receive" v-model="ammount_to_receive"
                              :class="isDark ? 'txn-cards-dark' : 'txn-cards-light'"
                              style="outline: none; height: 60px; padding-right: 25px!important; position: relative; border-radius: 15px; width: 100%; margin-top: 10px; padding-left: 15px;" />
                            ...
                            <span style="font-size: 14px;" :class="isDark ? 'text-dark' : 'text-light'">You will receive
                              this amount in
                              <span style="font-weight: 600; font-size: 14px;"
                                :class="isDark ? 'card-title' : 'card-title--light'">{{
                                  offer.trading_pair?.crypto?.token?.symbol }}</span>
                            </span>
                          </div>
                        ...
                        <div class="px-5 mb-3" style="justify-content: space-between;">
                          <v-btn @click="buyDirectlyDialog = true" :disabled="isButtonDisabled"
                            style="width: 100%; height: 50px; margin-bottom: 10px; font-weight: 600; border: 1px solid  #5892FF; background: inherit; border-radius: 16px; color: #2873FF; letter-spacing: 0px; text-transform: unset;">Buy
                            Directly Using Fiat Wallet</v-btn>

                          <v-dialog v-model="buyDirectlyDialog" width="auto">
                            <v-card max-width="400" :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                              style="border-radius: 15px; padding: 15px;">
                              <span class="text-center" style="font-size: 16px; font-weight: 600;">Confirm
                                Transaction</span>
                              <span style="font-size: 14px;">By clicking the confirm button, {{
                                offer?.countryCurrencyName }}{{ formatBalance(amount_to_pay) }} will be deducted from
                                your fiat balance and you will receive
                                <span style="font-weight: 600;"> {{ ammount_to_receive }} {{
                                  offer.trading_pair?.crypto?.token?.name }} </span>
                              </span>
                              <template v-slot:actions>

                                <v-btn class="ms-auto" @click="buyDirectlyDialog = false"
                                  style="text-transform: capitalize; letter-spacing: 0px; font-weight: 600; background: grey; border-radius: 8px !important;  width: 80px; color: white;">Cancel</v-btn>

                                <v-btn @click="Buy_OfferDirectly()" class="primary-btn1"
                                  style="text-transform: capitalize; letter-spacing: 0px; font-weight: 600; width: 80px; border-radius: 8px !important; color: white;">Confirm</v-btn>

                              </template>
                            </v-card>
                          </v-dialog>

                          <v-btn class="primary-btn1" @click="sellerDialog = true" :disabled="isButtonDisabled"
                            style="width: 100%; height: 50px; margin-bottom: 10px; font-weight: 600; color: white">Chat
                            With Seller</v-btn>

                          <v-dialog v-model="sellerDialog" max-width="500">
                            <v-card :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
                              style="padding: 20px; border-radius: 10px;">
                              <span class="text-center mb-4" :class="isDark ? 'text-dark' : 'text-light'"
                                style="font-size: 14px;">By
                                continuing you will be buying {{ offer.trading_pair?.crypto?.token?.name }} at this rate
                              </span>

                              <div :class="isDark ? 'txn-cards-dark' : 'txn-cards-light'"
                                style="font-size: 14px; display: flex; justify-content: space-between; padding: 10px;">
                                ...
                                <div
                                  style="display: flex; flex-direction: column; line-height: 30px; align-items: self-end; font-weight: 600">
                                  <span> {{ formatBalance(amount_to_pay) }} {{ offer?.countryCurrencyName }} </span>
                                  <span> {{ formatBalance(ammount_to_receive) }} {{
                                    offer.trading_pair?.crypto?.token?.symbol }}</span>
                              ...
                              <span style="font-size: 14px; margin-top: 15px; margin-top: 10px">Select Payment
                                method</span>
                              ...
                              <v-menu>
                                <template v-slot:activator="{ props }">
                                  <v-btn @click.prevent="toggleChevron" v-bind="props"
                                    :class="isDark ? 'txn-cards-dark' : 'txn-cards-light'"
                                    style="width: fit-content; height: 50px; margin-top: 2px; border-radius: 10px; margin-top: 10px;  box-shadow: none; letter-spacing: 0px;width: 100%; display: flex; justify-content: space-between; text-transform: capitalize;">
                                    <span class="currency-list">{{ selectedPaymentMethod }}</span>
                                    <div style="display: flex; position: absolute; right: 1%">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        :class="['chevron-icon', { 'chevron-icon-rotated': isChevronToggled.value }, isDark ? 'close-btn' : 'close-btn-dark']">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                          d="M12 13.5858L16.2929 9.29289C16.6834 8.90237 17.3166 8.90237 17.7071 9.29289C18.0976 9.68342 18.0976 10.3166 17.7071 10.7071L12.7071 15.7071C12.3166 16.0976 11.6834 16.0976 11.2929 15.7071L6.29289 10.7071C5.90237 10.3166 5.90237 9.68342 6.29289 9.29289C6.68342 8.90237 7.31658 8.90237 7.70711 9.29289L12 13.5858Z" />
                                      </svg>
                                    </div>
                                ...
                                <v-list style="border-radius: 10px; max-height: 120px"
                                  :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'">
                                  <v-list-item v-for="(item, index) in marketplace" :key="index">
                                    <v-row dense style="display: block;">
                                      <v-col sm="12">
                                        <v-list-item-content>
                                          <v-list-item-title>
                                            <div style="display: flex; justify-content: flex-start;">
                                              <span class="currency-list my-2">

                                                {{ selectedCoinId === item.id ?
                                                  (item?.trading_pair?.fiat?.payment_methods?.length ?
                                                    item.trading_pair.fiat.payment_methods.join(', ') : 'Bank transfer') : ''
                                                }}
                                              </span>
                                            </div>
                                          </v-list-item-title>
                              ...
                              <span style="font-size: 14px; margin-top: 6px; font-weight: 600">Please read the advisers
                                terms before placing
                                order.</span>
                              <ul class="list-bullets-styled my-3" :class="isDark ? 'text-dark' : 'text-light'">
                                <li>Keep all communications and transactions within the platform. Avoid sharing personal
                                  information or
                                  conducting transactions outside the platform, as this increases the risk of scams.
                                </li>
                              ...
                              <v-btn @click="Buy_OfferP2P()" class="primary-btn1"
                                style="height: 50px; font-size: 16px; font-weight: 600; margin-top: 10px;">Place
                                order</v-btn>
              ...
              <div v-if="!filteredOffers.length"
                ...
                <span class="d-flex" :class="isDark ? 'text-dark' : 'text-light'" style="margin: auto;">No records
                  found</span>
              </div>
          ...
          <div class="ml-5 flex-lg-and-up hidden-sm-and-down"
            :class="isDark ? 'profile-cards-dark' : 'profile-cards-light'"
            style="padding: 10px; border-radius: 10px; width: 30%;">
            <span class="resend-code ml-1">Filter By:</span>
                  ...
                  <v-row dense style="max-width: 240px; height: 250px; overflow: scroll;">
                    <div v-for="(range, index) in priceLimits" :key="index" style="width: 100%;">
                      ...
                      <v-list-item @click="setSelectedLimit(range)" style="display: flex;">
                        ...
                        <span class="currency-list">{{ range.min }} - {{ range.max === Infinity ? 'Infinity' : range.max
                          }}</span>
            ...
            <v-btn @click="resetFilters()" class="primary-btn1 mt-3"
              style="width: 100%; color: white; border-radius: 8px !important; font-weight: 600;">Clear Filters</v-btn>
