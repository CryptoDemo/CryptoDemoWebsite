<template>
    <div>
        <div v-if="!Fiat_transactions.length" class="no-transactions" style="height: 400px; display: flex; justify-content: center;">
        ...
        <div v-for="(transaction, index) in Fiat_transactions" :key="index">
...
const Fiat_transactions = ref(pinia.state.Fiat_transactions || []);

const getFiatTransactions = async () => {
  isloading.value = true;
  try {
    const data = await allFiatTxn(pageNumber.value);
    if (data.success) {
        Fiat_transactions.value = data.data.result;
        console.log(Fiat_transactions.value)

    //     Fiat_transactions.value = filterByKey("id", [
    //     ...Fiat_transactions.value,
    //     ...data.data.result,
    //   ]);
    //   pageNumber.value += 1;
    //   pinia.setTransactionDetails(Fiat_transactions.value);
